...
def predict_image(image_path):
    if not os.path.exists(image_path):
        print(f"Error: file not found by path: {image_path}")
        return
    try:
    ...
    print(f"models chosen: {predicted_class}")

    img = Image.open(image_path)
    plt.imshow(img)
    plt.title(f"models chosen: {predicted_class}")
    plt.axis("off")
    plt.show()

predict_image("dataset/cats/cat9.jpeg")